...
nlohmann::json Database::Query(const std::string &sql) {
  ...
  if (rc != SQLITE_OK) {
    LOG_ERROR("SQL error in Query prepare: {} | SQL: {}", sqlite3_errmsg(db_),
              sql);
    return result;
  }
...
bool Database::RegisterUser(const std::string &username,
  ...
  std::string hashed = CryptoUtils::HashPassword(password, salt);
...
nlohmann::json Database::ListFiles(int user_id, int parent_id) {
  std::lock_guard lock(mutex_);
  sqlite3_stmt *stmt;
  const char *sql = "SELECT id, filename, filesize, is_dir, created_at FROM "
                    "files WHERE owner_id = ? AND parent_id = ?;";
  nlohmann::json result = nlohmann::json::array();
...
nlohmann::json Database::GetFile(int user_id, int parent_id,
                                 const std::string &filename) {
  std::lock_guard lock(mutex_);
  sqlite3_stmt *stmt;
  const char *sql = "SELECT id, owner_id, parent_id, filename, filesize, "
                    "is_dir, file_path, created_at FROM files WHERE owner_id = "
                    "? AND parent_id = ? AND filename = ?;";
  ...
  if (sqlite3_step(stmt) == SQLITE_ROW) {
    for (int i = 0; i < sqlite3_column_count(stmt); i++) {
      ...
      int colType = sqlite3_column_type(stmt, i);
      if (colType == SQLITE_INTEGER)
        result[colName] = sqlite3_column_int64(stmt, i);
      else if (colType == SQLITE_TEXT)
        result[colName] =
            reinterpret_cast<const char *>(sqlite3_column_text(stmt, i));
    }
...
nlohmann::json Database::GetFileById(int user_id, int file_id) {
  ...
  sqlite3_stmt *stmt;
  const char *sql =
      "SELECT id, owner_id, parent_id, filename, filesize, is_dir, file_path, "
      "created_at FROM files WHERE owner_id = ? AND id = ?;";
...
bool Database::AddFile(int owner_id, int parent_id, const std::string &filename,
                       size_t size, bool is_dir, const std::string &path) {
  std::lock_guard lock(mutex_);
  sqlite3_stmt *stmt;
  const char *sql =
      "INSERT OR REPLACE INTO files (owner_id, parent_id, filename, filesize, "
      "is_dir, file_path) VALUES (?, ?, ?, ?, ?, ?);";
...
bool Database::MoveFile(int owner_id, int file_id, int new_parent_id) {
  ...
  sqlite3_stmt *stmt;
  const char *sql =
      "UPDATE files SET parent_id = ? WHERE owner_id = ? AND id = ?;";
...
std::vector<nlohmann::json> Database::GetAllSubFiles(int user_id,
                                                     int parent_id) {
  std::lock_guard lock(mutex_);
  ...
  std::string sql =
      ...
      "  SELECT id, filename, is_dir, file_path FROM files WHERE owner_id = " +
      std::to_string(user_id) +
      " AND parent_id = " + std::to_string(parent_id) +
      "  UNION ALL "
      "  SELECT f.id, f.filename, f.is_dir, f.file_path FROM files f "
      "  INNER JOIN subfiles s ON f.parent_id = s.id WHERE f.owner_id = " +
      std::to_string(user_id) +
...
nlohmann::json Database::GetAllDirectories(int user_id) {
  std::string sql =
      "SELECT id, parent_id, filename FROM files WHERE owner_id = " +
      std::to_string(user_id) + " AND is_dir = 1;";
  return Query(sql);
}

nlohmann::json Database::SearchFiles(int user_id, const std::string &keyword) {
  std::lock_guard lock(mutex_);
  sqlite3_stmt *stmt;
  const char *sql = "SELECT id, filename, filesize, is_dir, created_at FROM files "
